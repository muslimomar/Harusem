package com.example.william.harusem.ui.dialog;
...
public class TwoButtonsDialogFragment extends DialogFragment {
    ...
    public static void show(FragmentManager fm, int message, MaterialDialog.ButtonCallback callback) {
        show(fm, null, Harusem.getInstance().getString(message), true, null, null, callback);
    }

    public static void show(FragmentManager fm, int message) {
        show(fm, null, Harusem.getInstance().getString(message), true, null, null, null);
    }

    public static void show(FragmentManager fm, int title, int message, MaterialDialog.ButtonCallback callback) {
        show(fm, Harusem.getInstance().getString(title), Harusem.getInstance().getString(message), true, null, null, callback);
    }
    ...
    public static void show(FragmentManager fm, int title, int message,
        ...
        show(fm, Harusem.getInstance().getString(title), Harusem.getInstance().getString(message), true,
                Harusem.getInstance().getString(positiveText), Harusem.getInstance().getString(negativeText), callback);
    }
