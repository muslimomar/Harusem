package com.example.william.harusem.util.imagepick;
...
import com.example.william.harusem.util.async.BaseAsyncTask;
import com.example.william.harusem.util.consts.SchemeType;
...
public class GetFilepathFromUriTask extends BaseAsyncTask<Intent, Void, File> {
    ...
    public File performInBackground(Intent... params) throws Exception {
        ...
        if (SchemeType.SCHEME_CONTENT.equalsIgnoreCase(uriScheme) && !isFromGoogleApp && !isKitKatAndUpper) {
            ...
            Cursor cursor = Harusem.getInstance().getContentResolver().query(uri, filePathColumn, null, null, null);
            if (cursor != null) {
